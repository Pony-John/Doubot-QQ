...
---
# Part 1 部署步骤
1. 安装最新版本python，下载本项目，并安装依赖包：
```
pip3 install -r requirements.txt
```
2. 安装mirai-http-api框架；
可在github搜索mirai-http-api自行安装。
3. 配置mirai-http-api；
```
...
```
4. 启动mirai并登录QQ；
在mirai安装目录打开cmd，输入以下命令启动mcl
```
.\mcl
```
启动成功后，输入以下命令登录QQ
```
login QQ账号 QQ密码 ANDROID_WATCH
```
5. 启动bot.py
```
python bot.py
...
打开
```
C:\Users\Admin\AppData\Local\Programs\Python\Python311\Lib\site-packages\revChatGPT\Official.py
```
修改ENGINE一行为
```
